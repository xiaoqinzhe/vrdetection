        ...
        for obj in img['objects']:
            ...
            for name in obj['names']:
                ...
                if label in alias_dict:
                    ...
                    if label!=alias_dict[label]:
                        print('object alias: ', label, '->', alias_dict[label])
                names.append(label)
            obj['names'] = names
            if len(names)>1: print('names>1: ', obj['names'])
    print('preprocess object end')
    ...
    for img in data:
        for relation in img['relationships']:
            ...
            if predicate in alias_dict:
                ...
                if predicate!=alias_dict[predicate]:
                    print('predicate alias: ', predicate, '->', alias_dict[predicate])
...
def main(args):
    ...
    ''' object, predicate alias dicts: alias -> original term. '''
    obj_alias_dict = {}
    ...
    # make sure all objects that are in relationship dataset
    # are in object dataset
    obj_rel_cross_check(obj_data, rel_data, True)
