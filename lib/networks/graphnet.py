    ...
    def _net(self):
        ...
        if self.if_pred_rel:
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                cls_feat = slim.fully_connected(tf.concat([cls_sub, cls_obj], axis=1), 2048)
                spt = slim.fully_connected(self._spatial_feature(self.rel_inx1, self.rel_inx2), 2048)
                fc_pred =self._net_roi_fc(rel_roi_conv_out, reuse=True)
                net = tf.concat([fc_sub, fc_obj, fc_pred], axis=1)
